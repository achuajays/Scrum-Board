...
export const ScrumBoard: React.FC = () => {
  ...
  const loadIssues = async () => {
    try {
      ...
      console.log('Loaded issues from database:', issues);
  ...
  const handleIssueUpdate = async (updatedIssue: Issue) => {
    try {
      ...
      console.log('Updating issue in database:', { issueId: updatedIssue.id, updateData });
  ...
  const handleAddIssue = async (newIssue: Omit<Issue, 'id' | 'created_at' | 'updated_at' | 'position'>) => {
    try {
      ...
      console.log('Adding new issue to database:', issueToInsert);
