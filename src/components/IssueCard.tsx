        ...
        <div className="flex items-center space-x-2">
          {issue.assignee_name && (
            <div className="flex items-center space-x-2">
              ...
              <div className={`w-6 h-6 bg-slate-300 rounded-full flex items-center justify-center text-xs font-medium text-slate-600 ${issue.assignee_avatar ? 'hidden' : ''}`}>
                {issue.assignee_name.charAt(0).toUpperCase()}
              </div>
              <span className="text-xs text-slate-600 font-medium">{issue.assignee_name}</span>
            </div>
          )}
        </div>
